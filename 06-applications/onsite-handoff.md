...
## 0. 2026-08-10 현장 세션 — 결과

> **아래 0-0~0-3은 그날 밟았고, 결과를 이 박스에 요약한다.** 절차 자체는 참고용으로 남긴다.
>
> | | 결과 |
> |---|---|
> | 0-1 녹화 감시 | ✅ 살렸다. `camera_state`가 30초마다 갱신된다 |
> | 0-2 세그먼트 4초 | ✅ 적용. 라이브 윈도우 7초 → 32초 |
> | 0-3 재연결 실측 | ✅ 했다. 터널을 35초 내렸다 올리니 **새로고침 없이 저절로 돌아왔다** |
>
> **그런데 진짜 문제는 이 셋이 아니었다.** 오디오를 떼는 **ffmpeg 재발행 3개가 8/9
> 19:29~20:05에 죽은 뒤 아무도 살리지 않아 약 40시간 영상이 아예 없었다.** 그게
> "자꾸 끊긴다"의 최종 형태였다. 재발행 절차는 레포에도 문서에도 없었다 —
> 지금은 `cctv-setup.md`의 "카메라 오디오를 떼는 재발행"에 있다.
>
> 그 위에 **iPhone에서만 영상이 죽는 별개 문제**가 겹쳐 있었다. MediaMTX가 iOS
> User-Agent에만 쿠키를 요구하는데 브라우저가 교차 사이트로 그 쿠키를 보낼 방법이 없었다.
> Cloudflare Transform Rule에서 UA를 덮어써 해결했다(같은 문서 "iPhone만 400으로 죽는 이유").
>
> **자동 복구도 그날 붙였다.** `install-camera-relay.sh` 한 줄이면 로그인 시 자동으로 뜨고
> 죽으면 되살아난다(ffmpeg 강제 종료 16초, MediaMTX 재시작 30초 만에 복구 — 실측).
> macOS 26의 로컬 네트워크 권한 때문에 **네이티브 바이너리를 실행 파일로 둔 .app**이라야
> 통과한다. 셸 스크립트 번들은 안 된다.
>
> **다음 세션이 먼저 볼 것:**
> 1. **자동 로그인이 켜져 있는지 확인한다.** LaunchAgent는 사용자 세션에 붙어서, 재부팅 뒤
>    아무도 로그인하지 않으면 영상이 안 뜬다. (시스템 설정 › 사용자 및 그룹 › 자동으로 로그인)
> 2. 서버 녹화는 껐다. 영상은 SD카드에만 남고 맥에는 저장하지 않는다. 다만
>    `guest-guide.html`의 "7일 뒤 자동 파기"를 집행하던 주체가 사라졌으니, SD가 실제로
>    며칠을 담는지 Tapo 앱에서 확인해 문구와 맞춰야 한다.
