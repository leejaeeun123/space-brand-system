...
| `createTrigger()` | 15분 트리거 설치. 같은 함수의 기존 트리거를 지우고 다시 만들어 **중복 설치되지 않는다** |

> `reprocessMessages()`는 등록을 예약번호로, 취소를 멱등성으로 거르므로 이미 반영된 건이
> 중복되지 않는다. **단 예약번호를 못 뽑은 메일은 중복 등록될 수 있다** — `p_booking_no`가
> null이면 걸러낼 열쇠가 없다. 먼저 `auditReservations()`로 확인한다.
