...
**조작 알림(I) — 전 구간 실증됐다.** 2026-08-07 실기기로 확인:
...
### 현장 조작 판별 — 2026-08-08 실증됨

마지막까지 남았던 항목이다. **진짜 현장 조작이 잡혔다.**

| | |
|---|---|
| 사건 | 형운이 현장에서 에어컨을 직접 켰다(우리 API를 거치지 않음) |
| 잡힌 것 | `08:47:01 id=127 에어컨 onsite "전원 꺼짐 → 켜짐"` |
| 방향 | **맞음** — 기준선 OFF, 판독값 ON |
| 확인 | 2026-08-08 형운 본인 확인 |

다만 **10분 늦게 잡혔다.** 그 시간엔 예약이 없어 ThinQ 상태를 600초에 한 번만 물어보므로
(`IDLE_THINQ_MAX_AGE_SECONDS`), 08:47은 **조작 시각이 아니라 우리가 알아챈 시각**이다. 이걸
시각 하나로 찍었더니 그게 조작 시각처럼 읽혀 방향이 뒤집힌 것 아니냐는 의심을 받았다 — 그래서
알림이 이제 관측 구간을 찍는다(`08:37~08:47 사이`).

**같은 시간대의 또 한 건은 아직 안 갈렸다** — `08:45:01 id=126 SiHAS 천장 조명 onsite
"전원 켜짐 → 꺼짐"`. 해석이 둘이다:

- **사람이 껐다** — 같은 시간대에 현장에 사람이 있었으니 가장 자연스러운 설명이다.
- **우리 명령의 늦은 보고** — 08:36:44에 우리가 `power_off`를 보냈다. 큐 TTL이 60초라
  명령이 08:45에 실행됐을 리는 없지만, 08:36:50쯤 꺼졌는데 **에이전트가 8분 뒤에야
  `stat`을 올렸다면** 관측만 늦은 것이다(그날 아침 바닥 조명은 실제로 연결이 불안정했다).

둘을 가르려면 그 시각에 SiHAS 조명을 손으로 껐는지만 기억하면 된다. 갈리면 여기 적을 것.
