...
비밀번호 게이트는 없다 — 손님을 가르는 것은 비밀번호가 아니라 예약 시간 창이다. (처음 근거는
"현관 비밀번호가 사이트 루트에 이미 평문 공개"였는데, 그 값도 2026-08-17부터 예약 시간에만
내려간다. 그래도 결론은 같다.) 대신 등록·해제·CCTV는 서버(`auth.ts`)가 여전히 잘라낸다:
...
**앞뒤 공백은 잘라 심고, 공백만 있는 값은 빈 값으로 거부한다**(`20260818010000`) —
붙여넣기로 공백이 딸려 들어가면 증상이 세-자리 동기화 실패와 똑같아 오진을 부르기 때문이다.
상한은 없지만 bcrypt가 72바이트까지만 보므로 화면(`admin.html`)도 72자에서 자른다 —
72자 넘는 값은 심지 말 것.
처음엔 16자 바닥이 있었지만, 값을 자주 바꾸는 운영으로 가면서 없앴다: 16자를 매번 새로 만들어
세 자리에 옮기는 일이 회전 자체를 미루게 만들었다. 해싱(bcrypt cost 12)과 pgcrypto 스키마
위치는 함수가 알아서 처리하므로 `extensions.crypt(...)`를 손으로 적을 필요가 없다 — 그 스키마는
프로젝트마다 달라서 손으로 적으면 틀린 쪽에서 실패한다.

**반드시 새 값으로 바꾼다.** 기존 값은 마이그레이션 `20260803000000`에 평문으로 커밋돼 git
이력에 영구히 남아 있다 — 회전하지 않으면 이 작업의 절반이 무의미하다.

> ⚠️ **길이 바닥이 없으니 남는 방어는 시도 제한(10분/10회) 하나다.** 짧은 값을 쓰기로 했다면
> 두 가지를 지켜야 그 선택이 성립한다 — 회전 주기를 실제로 지키는 것, 그리고 **현관 비밀번호와
> 숫자를 공유하지 않는 값**을 쓰는 것. 현관 값은 사이트에 평문으로 나가므로(US-4.1) 그것의
> 재배열이면 탐색 공간이 순열 수로 줄어든다.
...
>
> ⚠️ **뒤집으면 이 편의가 위험이 된다** — Edge 시크릿이 뒤처진 채 두면 아침 문자가 옛 값을
> 매일 다시 내보내는데, 그 옛 값은 어드민 로그인(DB 해시)에는 안 먹어도 `control`·`claim`
> Edge Function(주민번호 reveal 포함)에는 **여전히 유효하다** — 둘 다 같은 시크릿을 비교하기
> 때문이다(auth.ts). 즉 회전을 반쯤 하면 '살아 있는 옛 열쇠'가 문자로 재배포된다.
> 2)단계를 했으면 이 3)단계를 같은 자리에서 끝낸다.
...
**`rpi-bridge` 끊긴 동안의 상태 재동기화 — 2026-08-16 실기기로 통과했다.**

`pollHttpState`는 진작 구현돼 있었는데 아무도 부르지 않아, 브릿지가 끊긴 동안 조명은 계속
제어됐지만(AWS 경로 + HTTP 우회) 화면 상태는 명령을 누를 때만 갱신되고 5분 재동기화가 죽는
문제가 있었다. `index.js`의 5분 주기에 `isBridgeUp() === false`일 때만 도는 HTTP 폴링을
얹었다(`pollBridgeDownState`). 빈 공간에서 실측:

1. tailscale을 실제로 내려 `rpi-bridge`를 끊었다 — 에이전트가 `끊김` 감지 확인
2. 브릿지가 끊긴 채로, 기기 하나를 HTTP로 직접 켰다(MQTT 전혀 안 거침) — DB는 여전히
   `꺼짐`으로 남아 실제 상태와 어긋난 채가 됐다
3. 다음 5분 주기가 돌자 20초 안에 DB가 `켜짐`으로 정확히 갱신됐다 — HTTP 폴링이 그 불일치를
   실제로 잡아냈다
4. tailscale을 복구하자 브릿지가 재연결되며 기존 `queryInitialState`가 자동으로 다시 돌아
   실제 상태(껐다)로 정확히 재동기화됐다 — 두 경로가 서로 덮어쓰며 싸우지 않았다

**power는 실패해도 절대 안 건드리고, online만 응답 없을 때 false로 내리도록
`pollHttpState`를 같이 고쳤다**(예전엔 실패 시 아무것도 안 썼다). retained LWT가 브릿지
끊긴 동안 거짓으로 '연결됨'을 유지하던 문제를 이걸로 메운다.
...
**제습(`AIR_DRY`) 모드의 온도 하한도 실측 아님**(2026-08-15 추가). 하한을 거는 모드에
제습을 넣었지만, 실기기가 **제습 상태에서 `set_temp`를 받아들이는지**는 확인 못 했다 —
기종에 따라 제습에선 목표온도를 무시하거나 범위가 좁아지는 경우가 있다.

**무시될 때의 증상이 '실패'가 아니라 '성공 반복'이라는 점이 중요하다.** ThinQ가 200을
주면 `issue()`는 성공으로 닫히므로 실패 경로(`startTempFloor({force:true})` 백오프)를
아예 안 탄다. 시계가 해제되고, 다음 틱에 온도가 여전히 18도라 시계가 다시 돌고, 5분 뒤
또 보낸다 — 채널에는 **「자동 · 온도 하한 24도」가 5분마다 무한 반복**되는데 방은 계속
18도다. 스윕과 달리 `temp_floor`는 알림을 묶는 `silent` 플래그가 없어 전부 나간다.
'고쳤다'는 글이 반복되는 모양이라 읽는 사람이 의심할 이유가 없는 게 이 고장의 함정이다.

그래서 다음 현장 방문의 확인은 **한 번 떴다 멈추는가**를 본다:

1. 이용 중(임시 예약)에 제습 18도로 두고 5분 기다린다
2. 「자동 · 온도 하한」이 한 번 뜨고 어드민 목표온도가 24도로 바뀌면 → 정상
3. 5분 뒤 **같은 알림이 또 뜨면** → 기기가 명령을 삼킨 것이다. 제습을 `FLOOR_MODES`에서
   빼거나, 하한 대신 모드를 `COOL`로 바꾸는 쪽으로 방향을 다시 잡는다

냉방(`COOL`)에서 동작하는 것은 08-14 17:14·17:58 두 번의 실측으로 확정돼 있다.
...
- **알림 순서** — `claimPending`의 반환 순서가 보장되지 않던 것을 정렬로 확정했다. 지금까지
  ...
  페이지를 회수하기 시작하면 깨진다.
- **어드민 로그인의 숫자 키패드**(2026-08-18) — `admin.html`의 비밀번호 칸에 `inputmode="numeric"`을
  기본값으로 두고, 그 아래 '영문·기호 키보드로 바꾸기' 전환구를 붙였다. 전환구의 배선은
  크로미움(데스크톱 마우스)·크로미움(아이폰 에뮬레이션 탭)·**웹킷(아이폰 에뮬레이션 탭)** 셋에서
  실제 포인터로 확인했다 — 탭 뒤에도 `mousedown` → `click`이 순서대로 오고, 포커스가 입력에
  남고, 치던 값이 안 지워지고, 선택이 새로고침 뒤에도 유지된다.
  **남은 건 하나다: 진짜 휴대폰에서 숫자 키패드가 실제로 뜨는지, 전환구를 눌렀을 때 소프트
  키보드가 실제로 바뀌는지.** 소프트 키보드 자체는 에뮬레이션에 없어서 기기에서만 보인다.
  현장 맥/휴대폰이 있을 때 `https://…/admin`을 열어 한 번 보고 이 줄을 결과로 고친다.
