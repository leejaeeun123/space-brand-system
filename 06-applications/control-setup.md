...
**예약 자동화(H) — 기기 쪽은 실증됐고, 예약 연동만 남았다.** 2026-08-07 23:5x 실기기로 확인:

- 에어컨 프로파일에 `COOL` 있음, `temp {min:18, max:30, step:0.5}` → 26·24 모두 격자에 맞음
- **전원→모드→온도 순서가 목표온도를 지킨다** — 실행 후 `mode=COOL, target_temp=26` 유지.
  모드 변경이 온도를 되돌리는 기종이 아니었다(그래도 순서는 유지한다 — 기기가 바뀌면 다시 문제가 된다)
- 조명은 큐→에이전트→기기→`stat` 보고 전 구간이 실제로 동작(`power=ON` 반영 확인).
  **손님 페이지(G)의 마지막 체크박스도 이걸로 함께 확인됐다**

아직 못 본 것은 **예약 연동 그 자체**다 — 입실 15분 전·퇴실 시각에 pg_cron이 이걸 자동으로
띄우는지는 실제 예약이 그 창을 지나가야 알 수 있다. 스윕과 온도 하한은 `isOccupied`가 참일
때만 도므로 예약 없이는 아예 실행되지 않아, 지금은 검증할 방법이 없었다.
...
아직 못 본 것은 **진짜 현장 조작이 잡히는지**다. 벽 스위치를 눌러봐야 알 수 있고, 이건 현장에
사람이 있어야 한다. 함께 확인할 것: 대조 창 120초가 실제 조명 반영 지연보다 넉넉한지 —
짧으면 우리가 켠 조명이 매번 현장 조작으로 알려져 알림 전체를 믿을 수 없게 만든다.
(2026-08-07 실측에서 조명 반영은 12초 안에 끝났으므로 여유는 있어 보이나, 에이전트가 밀렸을
때의 최악 경로는 재현하지 못했다.)
