...
**예약 자동화(H)는 실제 예약으로 검증하지 못했다.** 시각 판정(KST 파싱·준비/퇴실/스윕 창·
다음 예약 예외)은 `deno test`로 9개 항목을 확인했고 UTC 런타임에서도 통과한다. 하지만
**실제 예약 데이터로 기기가 움직이는지는 못 봤다** — pg_cron이 배포되고 예약이 그 창을
지나가야 확인된다. 특히 이 세 가지는 아직 모른다:

- 에어컨 프로파일에 `COOL` enum이 실제로 있는지(없으면 로그에 에러를 남기고 모드를 건너뛴다)
- 26도·24도가 그 기기의 min/max/step 격자에 맞는지
- 전원→모드→온도 순서가 실기기에서 정말 목표온도를 지키는지(모드 변경이 온도를 되돌리는 기종 대비)

**조작 알림(I)은 현장 조작 판별을 실제로 검증하지 못했다.** 메시지 묶기는 `deno test`로 7개
항목을 확인했지만, **벽 스위치를 눌렀을 때 정말 '현장 조작'으로 잡히는지**는 현장 맥과 실기기
앞에서만 확인된다. 특히 대조 창 120초가 실제 조명 반영 지연보다 넉넉한지는 실측이 필요하다 —
짧으면 우리가 켠 조명이 매번 현장 조작으로 알려지고, 그건 알림을 믿을 수 없게 만든다.
