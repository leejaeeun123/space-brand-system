...
### 2026-08-08 보강분 — **아직 한 줄도 실행되지 않았다**

리뷰에서 나온 결함을 고친 묶음이다. 로컬에서 `deno test`(42건)·`deno lint`·`deno check`만
통과했고, **마이그레이션도 안 올라갔고 함수도 재배포 안 됐다.** 그러니 아래는 전부 미검증이다.
...
| 무엇 | 어떻게 | 왜 봐야 하나 |
|---|---|---|
| 유휴 경보 | 예약 없는 시간에 조명 하나를 켜두고 1분 기다린다 | 새로 도는 유일한 규칙이다. 안 뜨면 `device_events`에 `kind='idle'` 행이 있는지부터 본다(있으면 알림, 없으면 제약) |
