...
- [ ] QR을 찍으면 건수와 날짜 범위가 뜬다 — 이 단계에서는 아직 **아무것도 안 바뀐다**
...
**청소 완료 QR(L)은 라이브에서 한 번도 안 돌렸다.** 시각 판정(자정을 넘기는 예약 포함)과
역할 판정은 순수 함수 테스트로 잡히지만(`completion.test.ts` 9개 · `auth.test.ts` 15개),
아래 셋은 실제 DB·채널 앞에서만 확인된다:

- 일괄 갱신이 어드민 화면의 청소 체크에 그대로 보이는가 (같은 컬럼이라 보여야 한다)
- 첫 스캔에서 밀린 예약이 몇 건 잡히는가 — 숫자가 예상보다 크면 **누르지 말고** 어드민에서
  먼저 확인한다. 되돌리려면 어드민에서 한 건씩 꺼야 한다
- 0건 알림이 실제로 채널에 올라오는가 — 안 올라오면 "QR 고장"과 "할 게 없었음"이 구분되지 않는다
- **담당자가 쓰는 스캐너 앱이 URL 프래그먼트를 살려 주는가** — 이게 안 되면 기능 전체가
  현장에서만 실패한다. 안 살아나면 토큰 전달 방식을 다시 정해야 한다(경로 세그먼트 등)

L-4를 밟기 전에는 **현장에 QR을 붙이지 않는다.**
