...
**청소 완료 QR(L) — 서버까지는 확인됐고, 사람이 찍는 구간은 아직이다.**

2026-08-09 배포 후 실제 함수에 대고 확인한 것:
...
아직 확인 못 한 것 넷. 전부 **사람이 폰으로 찍어야** 확인된다:

- **담당자가 쓰는 스캐너 앱이 URL 프래그먼트를 살려 주는가.** iOS 카메라·Safari는 살리지만
  카카오톡·네이버 인앱 브라우저는 확인된 바 없다. 안 살아나면 "주소가 올바르지 않아요"만
  뜨고 기능 전체가 **현장에서만** 실패한다 — 그때는 토큰 전달 방식을 다시 정해야 한다
  (경로 세그먼트 등). 아무 스캐너나 한 번 되는 걸로 넘기면 이걸 못 잡는다
- 버튼을 눌렀을 때 일괄 갱신이 실제로 도는가
- 그 결과가 어드민 화면의 청소 체크에 보이는가 (같은 컬럼이라 보여야 한다)
- 알림이 Mattermost에 실제로 도착하는가 — **0건 알림 포함.** 안 올라오면 "QR 고장"과
  "할 게 없었음"이 채널에서 구분되지 않는다

위 넷을 밟기 전에는 **현장에 QR을 붙이지 않는다.**
