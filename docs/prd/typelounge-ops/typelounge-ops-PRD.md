...
- [ ] **비밀번호 회전 완료** — 해시 전환(1단계 도구·검증 함수)은 배포됐지만 회전(2~4단계)은 미실행. git 이력에 평문으로 남은 옛 비밀번호가 아직 유효하다. 게이트 입력칸 `maxlength="12"`·`inputmode=numeric` 충돌은 2026-08-14 해소됐다(maxlength 64) — 남은 것은 회전 실행 자체다.
...
| Prototype or Design | 없음 |
| Additional Details | 근거: `supabase/functions/_shared/throttle.ts:21-94`, `20260813100000_auth_throttle.sql`. 게이트 로그인의 PostgREST RPC 경로는 이 제한 밖이었으나, 2026-08-14 `20260814150000_admin_check_throttle.sql`이 검증 뿌리(`admin_check`)에 같은 규칙을 얹어 해소했다 — 실효는 해시 전환(회전)이 라이브에 적용된 뒤부터다. |
