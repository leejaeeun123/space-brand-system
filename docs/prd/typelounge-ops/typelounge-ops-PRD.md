...
> 손님 제어를 비밀번호가 아니라 **예약 시간 창**으로 서버가 자르면, 게이트 없이도 안전하다. 처음 근거는 현관 비밀번호가 이미 가이드에 평문 공개라 게이트가 장벽이 아니라는 것이었고, 2026-08-17에 그 값도 시간 창 뒤로 들어갔지만 결론은 같다 — 가르는 것은 비밀번호가 아니라 시간이다.
...
- **손님 제어 페이지에 비밀번호 게이트를 두지 않는다.** 의도적 제거(#22) — 손님을 가르는 것은 예약 시간 창이고, 권한은 서버(`auth.ts`)가 자른다. (처음 근거였던 "현관 비밀번호가 사이트에 평문 공개"는 2026-08-17에 사라졌다.)
...
| ID | 인수 조건 |
| --- | --- |
| AC-1 | 가이드(`/`)의 현관 비밀번호·와이파이는 **예약 시간에만** 보인다 — 값은 HTML에 없고 `guide` action이 게이트(입실 안내 문자와 같은 시각 ~ 퇴실)를 통과한 요청에만 내려준다. CCTV 고지와 문의 연락처는 시간 창 밖에서도 보인다. |
...
| AC-3 | 온도 하한은 이용 중 + 켜짐 + 5분 경과 + 모드가 COOL/AIR_DRY/AUTO일 때만 강제된다 — 기준은 모드 이름이 아니라 압축기를 돌려 실내를 내리는가다(제습도 그렇다). HEAT에 24도를 밀면 난방이 세져 목적과 반대가 되므로 모드를 모르면 강제하지 않는다. |
