...
export async function issue(
  ...
  try {
    ...
    // 채널에 '퇴실 종료 성공'만 남는다. `command.ts`가 이 차이를 흐리지 말라고 못박은 것과
    // 같은 이유로 장부에도 그대로 남긴다.
    //
    // 다만 **표에는 안 찍는다**(#31). 조명 명령엔 항상 붙어 있어 정보가 되지 않았다 —
    // 정말 안 된 경우는 스윕 창이 끝날 때 `enforce.ts`가 실패로 기록해 ⚠️로 띄운다.
    // 장부에 남기는 건 화면을 위해서가 아니라 나중에 원인을 추적할 때 필요해서다.
