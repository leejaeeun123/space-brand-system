...
- **실제 배포 후의 접수 왕복.** 로컬 테스트는 순수 함수(검증·조립)까지만 돈다.
  `db push`·`functions deploy`를 한 뒤 `/apply`에서 실제로 한 번 보내 확인해야 한다.
- **Mattermost 실제 렌더.** 표·인용 블록이 채널에서 의도대로 보이는지는 한 통을 받아봐야 안다.
- 신청 내역을 읽는 경로는 지금 **Supabase 대시보드뿐이다.** 어드민 화면은 없다 —
  건수가 쌓여 대시보드가 불편해지면 `admin_list_*` RPC + 탭을 같이 만든다.
