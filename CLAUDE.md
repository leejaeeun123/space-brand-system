...
- **어드민 비밀번호를 공개 페이지에 넣지 않는다.** 그 값은 기기뿐 아니라 예약자 이름·연락처를 여는
  `admin_*` RPC의 열쇠다. 손님 페이지(`guest-control.html`)는 비밀번호가 없고(손님을 가르는 것은
  비밀번호가 아니라 예약 시간 창이다 — 처음 근거였던 "현관 비밀번호가 사이트 루트에 평문 공개"는
  2026-08-17에 사라졌지만 결론은 같다), 할 수 있는 일은 서버(`auth.ts`)가 자른다 —
  클라이언트에서 버튼을 감추는 건 방어가 아니다.
  **다만 청소 담당자에게는 아침 문자로 그 값을 보낸다**(형운 결정 2026-08-17) — 조명·냉난방·예약
  현황·CCTV가 필요한 내부 인력이라는 판단이고, `cleaning/templates.ts`가 이미 같은 근거로
  예약자 이름을 싣는다. 그 블록을 `Outgoing.body`에 넣지 않는다 — body는 DB 장부와 Mattermost
  채널로도 가므로, 넣으면 비밀번호가 장부에 영구히 남고 채널에 평문으로 올라간다.
  문자 발송 직전에만 붙이는 `Outgoing.smsOnly`가 그 경계다.
- **이용 안내 페이지의 현관 비밀번호·와이파이를 HTML로 되돌리지 않는다.** `guest-guide.html`은
  소스가 공개되므로 값을 두고 JS로 감추면 소스 보기로 읽힌다. 값은 `handlers/guide.ts`에 있고
  예약 시간(입실 안내 문자와 같은 시각)에만 내려간다. **예약이 연달아 붙은 날은 현관 비밀번호만
  앞 이용이 끝날 때까지 추가로 지연된다**(`guideGate`, 2026-08-18) — 리드타임 10분이 앞 손님의
  마지막 10분이라, 그때 내려주면 다음 손님이 이용 중에 문을 연다. 이 지연을 풀지 말 것. **CCTV 고지와 문의 연락처는 게이트 밖에
  둔다** — 고지는 현장 안내판이 없는 동안 §25의 유일한 표면이고, 연락처는 게이트가 닫혔을 때
  손님의 마지막 경로다.
