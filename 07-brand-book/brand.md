...
브랜드 식별은 **3색(크림·잉크·오렌지)** 으로 압축. **레이어 분리** — 디지털·마케팅은 컬러블록 과감하게, 실내 사이니지는 우드·브라스·블랙 재질 중심으로 차분하게(가구를 이기지 않게).
...
| **크림** | `#F4E9C8` | `spine/cream` | **메인** — 밝은 배경·레터링 |
| **잉크 블랙** | `#16130F` | `spine/ink` | **메인** — 본문 텍스트·블록 |
| 포레스트 그린 | `#235C42` | `block/green` | 세컨더리 블록(변주) |
| 코발트 블루 | `#1B4FA0` | `block/cobalt` | 세컨더리 블록(변주) |
| 브라스 | `#B0883C` | `material/brass` | 재질 포인트 — 브러시드 놋쇠 레터·디테일 |
| 웜우드(월넛) | `#8A5A33` | `material/walnut` | 재질 포인트 — 가구·우드 사인 |

### 사용 비율
```
메인+엑센트 (cream + ink + orange)   █████████████████  ~70%
세컨더리 블록 (green / cobalt)        █████              ~20%
재질 (brass / walnut, 사진·공간)      ███                ~10%
```
한 화면에 컬러블록은 **1~2개까지**.
...
| 배경 | 요소 | 용도 | 대비 |
|---|---|---|---|
| cream | ink / orange | 본문·정보 | ink on cream ✅ AAA |
| ink | cream / orange | 야간·포스터 | cream on ink ✅ AAA |
| orange | cream / ink | 메인 키비주얼 | cream on orange ✅ AA(대형만) |
| green / cobalt | cream | 변주 블록 | ✅ AA |

> **금지**: 오렌지·브라스를 **작은 본문 텍스트 색**으로 사용(대비 부족). 면·블록·대형 타이포·로고에만.
...
미드센추리 기하 라인 픽토그램. 일관된 그리드·선 굵기로 "고른(curated)" 인상.
...
- **방향 화살표**: 사각형을 쌓아 만든 **도형 조립 화살표**(미드센추리 톤).
...
미드센추리 그래픽 무드. 코어 무드(미드센추리·웜우드)를 바우하우스·스위스·기하로 표현. **그루비/사이키델릭 배제**.

### 비주얼 형용사 (지향)
볼드·웜 / 기하 / 정제된 / 물성 있는 / 절제된 위트(면이 겹쳐 생기는 교집합).

### Do
- **컷아웃 콜라주**: 미드센추리 가구를 색면·블롭 위에 얹기(furn. 방식).
...
| 레이어 | 톤 |
|---|---|
| 디지털·마케팅 | 컬러블록 과감 — 볼드 MCM 그래픽 |
| 실제 공간 사이니지 | 우드·브라스·블랙 물성 중심 — 차분하게 |
