...
4차 1종 (2026-08-06) — `light(조명)` — 천장 배선 + 펜던트 갓(사다리꼴) + **빛 3줄**. 빛 3줄은 냉난방의 바람 물결·바베큐의 연기와 같은 어법.
**5종은 Figma 확정본이 정본** (2026-08-06) — `tv · aircon · speaker · trash · shoe`.
2026-07-26 세트 개정(선 3.0→2.7)이 Figma 안내문에 반영되지 않아 형태가 어긋나 있었고, **실제 인쇄·부착된 것은 Figma 쪽**이라 Figma 기준으로 맞췄다.
- 생성: `figma_to_picto.py` — Figma export(최종 배치 px)의 아웃라인 두께를 실측해 그리드 관례 2.7이 되는 배율로 확대·가운데 정렬. 종횡비·형태는 Figma 그대로, 선 두께는 세트와 동일.
- `gen_new_pictos.py`는 이 5종을 **건너뛴다**(`FIGMA_OWNED`) — 다시 돌려도 옛 기하로 덮이지 않는다. 그 안의 정의는 최초 작도 이력으로 보존.
- 안내문 재현 검증: 세로/가로 A5 7종의 픽토 폭이 Figma 실측과 0.02px 이내 일치.
